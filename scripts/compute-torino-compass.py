#!/usr/bin/env python3
"""Compass compute layer per provincia di Torino — fork di compute-compass.py.

Stessa pipeline: per ogni tipologia chiave (5 tipi) computa:
  - Metriche per zona Torino città (CAGR, yield, vol, momentum, level)
...
Output:
  data/computed/torino-compass.json — by_tipologia con 5 tipologie
...
OUT_FILE = OUT_DIR / "torino-compass.json"

CAPOLUOGO_CATASTO = "L219"
CAPOLUOGO_NAME = "Torino"
...
# Allineati al caption UI (torino-C-compass.html): 35/30/15/15/5, soglia 70, penalty -10.
...
def validate(payload):
    ...
    for tipo, data in payload["by_tipologia"].items():
        h = data["headline"]
        # Asserts allentati per Torino — provincia con copertura parziale
    ...
    payload = {
        "metadata": {
            ...
            "pool_composition": {
                ...
                "warning": "Pool combina zone OMI capoluogo + comuni provincia nella stessa normalizzazione min-max. Score relativi: una zona BUY a Torino NON è automaticamente paragonabile a una BUY a Modena.",
            },
        },
