#!/usr/bin/env python3
"""Compass compute layer per provincia di Bologna — fork di compute-compass.py.

Stessa pipeline: per ogni tipologia chiave (5 tipi) computa:
  - Metriche per zona Bologna città (CAGR, yield, vol, momentum, level)
...
Output:
  data/computed/bologna-compass.json — by_tipologia con 5 tipologie
...
OUT_FILE = OUT_DIR / "bologna-compass.json"

CAPOLUOGO_CATASTO = "A944"
CAPOLUOGO_NAME = "Bologna"
...
# Allineati al caption UI (bologna-C-compass.html): 35/30/15/15/5, soglia 70, penalty -10.
...
def validate(payload):
    ...
    for tipo, data in payload["by_tipologia"].items():
        h = data["headline"]
        # Asserts allentati per Bologna — provincia con copertura parziale
    ...
    payload = {
        "metadata": {
            ...
            "pool_composition": {
                ...
                "warning": "Pool combina zone OMI capoluogo + comuni provincia nella stessa normalizzazione min-max. Score relativi: una zona BUY a Bologna NON è automaticamente paragonabile a una BUY a Modena.",
            },
        },
