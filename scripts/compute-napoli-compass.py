#!/usr/bin/env python3
"""Compass compute layer per provincia di Napoli — fork di compute-compass.py.

Stessa pipeline: per ogni tipologia chiave (5 tipi) computa:
  - Metriche per zona Napoli città (CAGR, yield, vol, momentum, level)
...
Output:
  data/computed/napoli-compass.json — by_tipologia con 5 tipologie
...
OUT_FILE = OUT_DIR / "napoli-compass.json"

CAPOLUOGO_CATASTO = "F839"
CAPOLUOGO_NAME = "Napoli"
...
# Allineati al caption UI (napoli-C-compass.html): 35/30/15/15/5, soglia 70, penalty -10.
...
def validate(payload):
    ...
    for tipo, data in payload["by_tipologia"].items():
        h = data["headline"]
        # Asserts allentati per Napoli — provincia con copertura parziale
    ...
    payload = {
        "metadata": {
            ...
            "pool_composition": {
                ...
                "warning": "Pool combina zone OMI capoluogo + comuni provincia nella stessa normalizzazione min-max. Score relativi: una zona BUY a Napoli NON è automaticamente paragonabile a una BUY a Modena.",
            },
        },
