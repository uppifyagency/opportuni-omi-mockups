#!/usr/bin/env python3
"""Parser Statistiche Regionali AdE Emilia-Romagna — provincia di Reggio Emilia.

Fork di parse-volumi-ade-bologna.py. Stessa regione → COL_MAPS identico.
Differenze:
  - find_reggio_emilia_pages() invece di find_bologna_pages()
  - MACROAREE_KNOWN per Reggio Emilia (7 macroaree + capoluogo + totale)
  - p_boundary anti-cross-contamination (FIX §19.3 lezione Catanzaro)
  - OUT_JSON → reggio-emilia-volumi-timeseries.json

Reggio Emilia è alfabeticamente DOPO Ravenna e PRIMA di Rimini nei PDF SR Emilia-Romagna.

Output:
  data/volumi/reggio-emilia-volumi-timeseries.json
  data/volumi/parse-log-reggio-emilia.txt
...
OUT_JSON = PDF_DIR / "reggio-emilia-volumi-timeseries.json"
LOG_FILE = PDF_DIR / "parse-log-reggio-emilia.txt"

# Macroaree provincia di Reggio Emilia (verificate ispezionando SR2024 p.56).
# Stessa regione Emilia-Romagna di Modena/Bologna → stesso layout COL_MAPS riutilizzabile.
# Notare: capoluogo nei PDF è scritto "REGGIO NELL'EMILIA" (apostrofo unicode in alcuni PDF).
MACROAREE_KNOWN = {
    "DISTRETTO CERAMICHE",
    "PIANURA CENTRALE",
    "PIANURA EST",
    ...
    "ZONA COLLINARE",
    "ZONA MONTANA",
    "ZONA PEDECOLLINARE E VAL D'ENZA",
    "REGGIO NELL'EMILIA CAPOLUOGO",
    "REGGIO NELL'EMILIA",
}
...
def find_reggio_emilia_pages(pdf):
    """Trova p_prov, p_com di Reggio Emilia + p_boundary = inizio provincia successiva (Rimini).
    FIX §19.3 (lezione Catanzaro): senza boundary il parser ingerisce zone di provincia adiacente.
    Reggio Emilia è la PENULTIMA provincia in ordine alfabetico (segue Ravenna, precede Rimini).
    L'Indice viene skippato anche qui per sicurezza (le prime ~8 pagine non contengono dati).
    Match: "Reggio Emilia" OR "Reggio nell'Emilia" — entrambe le scritture compaiono nei PDF.
    """
    SKIP_FIRST = 8
    p_prov, p_com, p_boundary = None, None, None
    # Regex robusto per il nome (con apostrofo retto o curly, "Emilia" o "nell'Emilia")
    re_prov = re.compile(r'^\s*La provincia\s*[-–]\s*Reggio(\s+nell[\'’]?)?\s*Emilia', re.I | re.M)
    re_com  = re.compile(r'^\s*Il comune\s*[-–]\s*Reggio(\s+nell[\'’]?)?\s*Emilia',  re.I | re.M)
    for i, page in enumerate(pdf.pages):
        ...
        if p_com is not None and p_boundary is None:
            ...
            if m and m.group(2).lower() != 'reggio':
                p_boundary = i
    ...
    for pi in range(page_idx, end_page):
        ...
        for table in pdf.pages[pi].extract_tables() or []:
            ...
            for tr in table:
                ...
                # Normalize curly apostrophes to ASCII so both write forms match MACROAREE_KNOWN
                first = first.replace('’', "'")
                if first not in MACROAREE_KNOWN: continue
                row = {
                    "year": data_year,
                    "level": "provincia" if first == "REGGIO NELL'EMILIA" else "macroarea",
                    "name": first,
                }
...
def parse_zone(pdf, page_idx, p_boundary, sr_year, data_year, log):
    """Reggio Emilia SR (16 zone OMI incluse 2 rurali) tabella distribuita su ~2-3 pagine
    dopo 'Il comune - Reggio nell'Emilia'. Range esteso fino a p_boundary (Rimini)
    per non perdere zone, ma rispetta il boundary anti-cross-contamination.
    """
    rows = []
...
def parse_year(pdf_path, log):
    ...
    p_prov, p_com, p_boundary = find_reggio_emilia_pages(pdf)
    if p_prov is None or p_com is None:
        log.append(f"  ✗ Reggio Emilia pages not found")
        pdf.close()
        return None
...
def main():
    ...
    _province_total = "REGGIO NELL'EMILIA"
    log.append(f"Missing: {sorted(MACROAREE_KNOWN - {_province_total} - set(macro_set))}")
