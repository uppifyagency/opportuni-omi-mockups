#!/usr/bin/env python3
"""Bologna investor signal compute layer.

Fork di compute-modena-signals.py riconfigurato per provincia di Bologna:
capoluogo A944, ~55 comuni, GeoJSON dedicati. Stessa pipeline matematica.
...
Writes:
  data/computed/bologna-signals.json     (frontend-ready, pre-validated)
  data/computed/bologna-zone-series.json (per-zone full timeseries)
...
ZONE_GEOJSON = ROOT / "data" / "geojson" / "bologna-zone-omi.geojson"
PROVINCE_GEOJSON = ROOT / "data" / "geojson" / "bologna-province-comuni.geojson"
OUT_DIR = ROOT / "data" / "computed"
OUT_SIGNALS = OUT_DIR / "bologna-signals.json"
OUT_SERIES = OUT_DIR / "bologna-zone-series.json"
...
# Bologna's dataset is biennial 2014–2026 (7 expected datapoints). Zones with only
...
def province_metrics(rows: list[dict], geoj: dict) -> list[dict]:
    """Per-comune province ranking — restricted to comuni inside Bologna's province
    ...
    ranking with comuni that aren't actually in Bologna."""
...
def main():
    ...
    print(f"\n=== Bologna ({HEADLINE_TIPO} · {HEADLINE_OP}) ===")
    ts_acq = build_zone_series(rows, "A944", HEADLINE_TIPO, "acquisto")
    ts_aff = build_zone_series(rows, "A944", HEADLINE_TIPO, "affitto")
    ...
    fascia_acq = build_fascia_series(rows, "A944", HEADLINE_TIPO, "acquisto")
    fascia_metrics = {}
    ...
    h = signals["headline"]
    print(f"\n  HEADLINE:")
    print(f"    Bologna, {h['zone_count_current']} zone correnti")
