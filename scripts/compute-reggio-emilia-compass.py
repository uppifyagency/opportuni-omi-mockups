#!/usr/bin/env python3
"""Compass compute layer per provincia di Reggio Emilia — fork di compute-compass.py.

Stessa pipeline: per ogni tipologia chiave (5 tipi) computa:
  - Metriche per zona Reggio Emilia città (CAGR, yield, vol, momentum, level)
...
Output:
  data/computed/reggio-emilia-compass.json — by_tipologia con 5 tipologie
...
OUT_FILE = OUT_DIR / "reggio-emilia-compass.json"

CAPOLUOGO_CATASTO = "H223"
CAPOLUOGO_NAME = "Reggio Emilia"
...
# Allineati al caption UI (reggio-emilia-C-compass.html): 35/30/15/15/5, soglia 70, penalty -10.
...
def compute_tipologia(rows, tipo, zone_meta, prov_meta):
    ...
    # Phase 4: tag EMERGING (potenziale crescita) — FIX threshold-scientific 2026-05-17
...
def validate(payload):
    ...
    for tipo, data in payload["by_tipologia"].items():
        h = data["headline"]
        # Asserts allentati per Reggio Emilia — provincia con copertura parziale
    ...
    payload = {
        "metadata": {
            ...
            "scoring": {
                ...
                "note": "Score formula: sum(weights[k]*c[k]) / sum(weights[k] dove c[k] is not None) * 100. Se cagr<0, sottrai CAGR_NEGATIVE_PENALTY (clamped 0-100). Verdict: BUY se score>=Jenks_buy_threshold, AVOID se score<Jenks_avoid_threshold, WATCH altrimenti.",
            },
            ...
            "pool_composition": {
                ...
                "warning": "Pool combina zone OMI capoluogo + comuni provincia nella stessa normalizzazione min-max. Score relativi: una zona BUY a Reggio Emilia NON è automaticamente paragonabile a una BUY a Modena.",
            },
        },
