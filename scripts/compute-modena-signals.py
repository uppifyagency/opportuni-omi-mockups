#!/usr/bin/env python3
"""Modena investor signal compute layer.
...
Writes:
  data/computed/modena-signals.json     (frontend-ready, pre-validated)
  data/computed/modena-zone-series.json (per-zone full timeseries)
...
OUT_SIGNALS = OUT_DIR / "modena-signals.json"
OUT_SERIES = OUT_DIR / "modena-zone-series.json"
...
# Modena's dataset is biennial 2014–2026 (7 expected datapoints). Zones with only
...
def province_metrics(rows: list[dict], geoj: dict) -> list[dict]:
    """Per-comune province ranking — restricted to comuni inside Modena's province
    ...
    ranking with comuni that aren't actually in Modena."""
    ...
    out = []
    comuni_with_data = sorted({r["comune_catasto"] for r in rows} & province_codes)
    ...
    return sorted(out, key=lambda x: x["cagr"] if x["cagr"] is not None else 999, reverse=True)
...
def validate(signals: dict) -> None:
    ...
    assert -10 <= headline["modena_cagr_avg_pct"] <= 10
...
def main():
    ...
    print(f"\n=== Modena ({HEADLINE_TIPO} · {HEADLINE_OP}) ===")
    ts_acq = build_zone_series(rows, "F257", HEADLINE_TIPO, "acquisto")
    ts_aff = build_zone_series(rows, "F257", HEADLINE_TIPO, "affitto")
    ...
    fascia_acq = build_fascia_series(rows, "F257", HEADLINE_TIPO, "acquisto")
    fascia_metrics = {}
    ...
    signals = {
        "metadata": {
            ...
            "filters": {
                ...
                "applies_to": "top_growth, top_decline, headline.modena_cagr_avg_pct",
            },
        },
        "headline": {
            ...
            "modena_cagr_avg_pct": safe_round(sum(cagrs) / len(cagrs) * 100, 2) if cagrs else None,
            "modena_cagr_avg_pct_min_years": MIN_CAGR_YEARS,
    ...
    h = signals["headline"]
    print(f"\n  HEADLINE:")
    print(f"    Modena, {h['zone_count_current']} zone correnti")
    print(f"    Orizzonte dataset: {h['anni_orizzonte_dataset']} anni · Zone correnti: {h['anni_orizzonte_zone_correnti']} anni")
    print(f"    CAGR medio: {h['modena_cagr_avg_pct']:+.2f}% / anno (su {h['zone_count_cagr_reliable']} zone ≥{MIN_CAGR_YEARS} datapoint)")
