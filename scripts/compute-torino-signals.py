#!/usr/bin/env python3
"""Torino investor signal compute layer.

Fork di compute-modena-signals.py riconfigurato per provincia di Torino:
capoluogo L219, ~55 comuni, GeoJSON dedicati. Stessa pipeline matematica.
...
Writes:
  data/computed/torino-signals.json     (frontend-ready, pre-validated)
  data/computed/torino-zone-series.json (per-zone full timeseries)
...
ZONE_GEOJSON = ROOT / "data" / "geojson" / "torino-zone-omi.geojson"
PROVINCE_GEOJSON = ROOT / "data" / "geojson" / "torino-province-comuni.geojson"
OUT_DIR = ROOT / "data" / "computed"
OUT_SIGNALS = OUT_DIR / "torino-signals.json"
OUT_SERIES = OUT_DIR / "torino-zone-series.json"
...
# Torino's dataset is biennial 2014–2026 (7 expected datapoints). Zones with only
...
def province_metrics(rows: list[dict], geoj: dict) -> list[dict]:
    """Per-comune province ranking — restricted to comuni inside Torino's province
    ...
    ranking with comuni that aren't actually in Torino."""
...
def main():
    ...
    print(f"\n=== Torino ({HEADLINE_TIPO} · {HEADLINE_OP}) ===")
    ts_acq = build_zone_series(rows, "L219", HEADLINE_TIPO, "acquisto")
    ts_aff = build_zone_series(rows, "L219", HEADLINE_TIPO, "affitto")
    ...
    fascia_acq = build_fascia_series(rows, "L219", HEADLINE_TIPO, "acquisto")
    fascia_metrics = {}
    ...
    # (caso estremo: provincia appena estratta senza storia). Per Torino 2026 le
    ...
    signals = {
        ...
        "headline": {
            ...
            # Per Torino 2026 = 0 (rinumerazione totale 2024) → cagr_avg_pct viene dai
    ...
    h = signals["headline"]
    print(f"\n  HEADLINE:")
    print(f"    Torino, {h['zone_count_current']} zone correnti")
