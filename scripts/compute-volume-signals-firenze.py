...
VOL_JSON = ROOT / "data" / "volumi" / "firenze-volumi-timeseries.json"
MAP_JSON = ROOT / "data" / "volumi" / "zone-mapping-old-new-firenze.json"
SIG_JSON = ROOT / "data" / "computed" / "firenze-signals.json"
OUT_JSON = ROOT / "data" / "computed" / "firenze-volume-signals.json"
...
def main():
    ...
    # Firenze MAP_JSON: mapping macroarea → list of OMI codes (NEW = Sagona codes).
    # rel: "macroarea-to-omi" (1 macroarea → N OMI zones).
    macroarea_to_omi = {m['old']: m for m in json.loads(MAP_JSON.read_text())['mapping']}
    sig = json.loads(SIG_JSON.read_text())
    sig_by_zona = {z['zona']: z for z in sig['zone_metrics']}

    # === MACROAREE URBANE → replica volume metrics su ogni zona OMI contenuta
    # A differenza di Modena/Reggio (zone OMI 1:1), Firenze SR aggrega in macroaree.
    # Per ogni macroarea, replichiamo time series su tutte le sue OMI zone Sagona.
    by_zona = {}
    for r in vol['macroaree_urbane_series']:
        if r.get('level') != 'macroarea_urbana':
            continue
        macro = r['macroarea']
        omi_codes = macroarea_to_omi.get(macro, {}).get('new', [])
        for omi in omi_codes:
            # Sintetizza row "zone_series" shape per ogni OMI sotto la macroarea
            by_zona.setdefault(omi, []).append({
                'year': r['year'],
                'zona': omi,
                'denominazione': sig_by_zona.get(omi, {}).get('dizione', macro),
                'ntn': r.get('ntn'),
                'ntn_var_pct': r.get('ntn_var_pct'),
                'imi_pct': r.get('imi_pct'),
                'quotazione_eur_mq': r.get('quotazione_eur_mq'),
                'quotazione_var_pct': r.get('quotazione_var_pct'),
                '_source_macroarea': macro,
            })
    ...
    for zona_old, rows in by_zona.items():
        ...
        # For Firenze: zona_old is the OMI code itself (NEW Sagona). We retain the
        # _source_macroarea so the dashboard can group by macroarea and show provenance.
        source_macro = rows[-1].get('_source_macroarea', '')
        zone_metrics.append({
            "zona_old": zona_old,
            "zona_new": [zona_old],
            "denominazione": rows[-1].get('denominazione', ''),
            "rel": "macroarea_inheritance",
            "mapping_notes": f"Volume metrics inherited from macroarea: {source_macro}",
            "source_macroarea": source_macro,
    ...
    payload = {
        "metadata": {
            ...
            "source": "data/volumi/firenze-volumi-timeseries.json (PDF AdE Statistiche Regionali Toscana) + data/computed/firenze-signals.json (Sagona OMI quotazioni). Volume metrics inherited from macroaree urbane (no per-OMI-zone breakdown in SR PDFs).",
