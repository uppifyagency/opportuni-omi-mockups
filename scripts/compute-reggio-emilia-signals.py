#!/usr/bin/env python3
"""Reggio Emilia investor signal compute layer.

Fork di compute-modena-signals.py riconfigurato per provincia di Reggio Emilia:
capoluogo H223, ~55 comuni, GeoJSON dedicati. Stessa pipeline matematica.
...
Writes:
  data/computed/reggio-emilia-signals.json     (frontend-ready, pre-validated)
  data/computed/reggio-emilia-zone-series.json (per-zone full timeseries)
...
ZONE_GEOJSON = ROOT / "data" / "geojson" / "reggio-emilia-zone-omi.geojson"
PROVINCE_GEOJSON = ROOT / "data" / "geojson" / "reggio-emilia-province-comuni.geojson"
OUT_DIR = ROOT / "data" / "computed"
OUT_SIGNALS = OUT_DIR / "reggio-emilia-signals.json"
OUT_SERIES = OUT_DIR / "reggio-emilia-zone-series.json"
...
# Reggio Emilia's dataset is biennial 2014–2026 (7 expected datapoints). Zones with only
...
def province_metrics(rows: list[dict], geoj: dict) -> list[dict]:
    """Per-comune province ranking — restricted to comuni inside Reggio Emilia's province
    ...
    ranking with comuni that aren't actually in Reggio Emilia."""
...
def main():
    ...
    print(f"\n=== Reggio Emilia ({HEADLINE_TIPO} · {HEADLINE_OP}) ===")
    ts_acq = build_zone_series(rows, "H223", HEADLINE_TIPO, "acquisto")
    ts_aff = build_zone_series(rows, "H223", HEADLINE_TIPO, "affitto")
    ...
    fascia_acq = build_fascia_series(rows, "H223", HEADLINE_TIPO, "acquisto")
    fascia_metrics = {}
    ...
    h = signals["headline"]
    print(f"\n  HEADLINE:")
    print(f"    Reggio Emilia, {h['zone_count_current']} zone correnti")
