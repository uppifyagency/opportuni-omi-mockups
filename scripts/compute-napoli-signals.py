#!/usr/bin/env python3
"""Napoli investor signal compute layer.

Fork di compute-modena-signals.py riconfigurato per provincia di Napoli:
capoluogo F839, ~55 comuni, GeoJSON dedicati. Stessa pipeline matematica.
...
Writes:
  data/computed/napoli-signals.json     (frontend-ready, pre-validated)
  data/computed/napoli-zone-series.json (per-zone full timeseries)
...
ZONE_GEOJSON = ROOT / "data" / "geojson" / "napoli-zone-omi.geojson"
PROVINCE_GEOJSON = ROOT / "data" / "geojson" / "napoli-province-comuni.geojson"
OUT_DIR = ROOT / "data" / "computed"
OUT_SIGNALS = OUT_DIR / "napoli-signals.json"
OUT_SERIES = OUT_DIR / "napoli-zone-series.json"
...
# Napoli's dataset is biennial 2014–2026 (7 expected datapoints). Zones with only
...
def province_metrics(rows: list[dict], geoj: dict) -> list[dict]:
    """Per-comune province ranking — restricted to comuni inside Napoli's province
    ...
    ranking with comuni that aren't actually in Napoli."""
...
def main():
    ...
    print(f"\n=== Napoli ({HEADLINE_TIPO} · {HEADLINE_OP}) ===")
    ts_acq = build_zone_series(rows, "F839", HEADLINE_TIPO, "acquisto")
    ts_aff = build_zone_series(rows, "F839", HEADLINE_TIPO, "affitto")
    ...
    fascia_acq = build_fascia_series(rows, "F839", HEADLINE_TIPO, "acquisto")
    fascia_metrics = {}
    ...
    # (caso estremo: provincia appena estratta senza storia). Per Napoli 2026 le
    ...
    signals = {
        ...
        "headline": {
            ...
            # Per Napoli 2026 = 0 (rinumerazione totale 2024) → cagr_avg_pct viene dai
    ...
    h = signals["headline"]
    print(f"\n  HEADLINE:")
    print(f"    Napoli, {h['zone_count_current']} zone correnti")
